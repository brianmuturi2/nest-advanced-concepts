...
export class CoffeesService {
  ...
  async create(createCoffeeDto: CreateCoffeeDto) {
    console.time();
    const rewardsModuleRef = await this.lazyModuleLoader.load(() => import('../rewards/rewards.module').then(m => m.RewardsModule));
    const {RewardsService} = await import('../rewards/rewards/rewards.service');
